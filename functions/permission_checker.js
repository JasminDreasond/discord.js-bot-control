module.exports = async (guilds, authorID, bot_index, safeDS) => {
    ...
    if (!app_permissions.admin) {
        if (Array.isArray(safeDS.config.discord.bots[bot_index].superAdmin)) {
    ...
    // Is Bot Admin
    if (!app_permissions.admin) {
        if (Array.isArray(safeDS.config.discord.bots[bot_index].admin)) {
    ...
    // Is Role Admin
    if (!app_permissions.admin) {
        if (Array.isArray(safeDS.config.discord.bots[bot_index].adminRoles)) {
